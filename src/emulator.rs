...
use crate::{config::Config, util::UniErr, Args, system::System, framebuffers::sdl_engine::{PUMP_EVENT_INST_INTERVAL, SDL}, emulator_trace::TraceState};
...
fn record_bus_fault(sys: &System<'_, '_>, addr: Option<u32>, instruction_fetch: bool) {
    let escalated = sys.p.nvic.borrow_mut().signal_fault("BusFault");
    sys.p.scb.borrow_mut().record_bus_fault(addr, instruction_fetch, escalated);
}

fn record_usagefault_invstate(sys: &System<'_, '_>) {
    let escalated = sys.p.nvic.borrow_mut().signal_fault("UsageFault");
    sys.p.scb.borrow_mut().record_usagefault_invstate(escalated);
}
...
pub fn run_emulator(config: Config, svd_device: SvdDevice, args: Args) -> Result<()> {
    ...
    let deferred_irq = Rc::new(RefCell::new(None::<i32>));
    {
        ...
        sys.uc.borrow_mut().add_code_hook(0, u64::MAX, move |uc, pc, size| {
            ...
            let pc32 = pc as u32;
            trace_state.on_instruction(uc, pc32);
            unsafe {
                if busy_loop_stop {
        ...
        sys.uc.borrow_mut().add_intr_hook(move |uc, exception| {
            match exception {
                ...
                3 | 4 => {
                    ...
                    let sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone() };
                    record_bus_fault(&sys, Some(pc as u32), exception == 3);
                    CONTINUE_EXECUTION.store(true, Ordering::Release);
                    uc.emu_stop().unwrap();
                }
                18 => {
                    let sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone() };
                    record_usagefault_invstate(&sys);
                    CONTINUE_EXECUTION.store(true, Ordering::Release);
                    uc.emu_stop().unwrap();
                }
                _ => {
...

    {
        let p = sys.p.clone();
        let d = sys.d.clone();
        sys.uc.borrow_mut().add_mem_hook(HookType::MEM_UNMAPPED, 0, u64::MAX, move |uc, type_, addr, size, value| {
        ...
        let sys = System { uc: RefCell::new(uc), p: p.clone(), d: d.clone() };
        record_bus_fault(&sys, Some(addr as u32), !matches!(type_, MemType::WRITE_UNMAPPED | MemType::READ_UNMAPPED));
    ...
    if (pc & 1) == 0 {
        bail!("Reset vector 0x{:08x} does not have Thumb bit set", pc as u32);
    }
    ...
    sys.uc.borrow_mut().reg_write(RegisterARM::PC, pc).map_err(UniErr)?;
