...
| No USB CDC data bridge for console output | high | Partially Implemented | The emulator now runs cleanly to `100M` instructions and USB enumeration (SET_ADDRESS + SET_CONFIG) completes. Firmware routes its console to USB serial (`HAL_USE_SERIAL_USB TRUE`). Without a virtual USB host receiving CDC bulk OUT NAK'd frames, the firmware has no peer to transmit to. The next step is adding a CDC bulk IN data capture path so the firmware's console text can be read out of the emulator. DMA ADC1 and SPI2 reads at ~10.5M instructions confirm the firmware is in its runtime sensor-polling loop and has bypassed the early startup stalls entirely. |
...
- P0 (must-have for runtime):
	- **USB CDC bulk IN data capture** — firmware routes its console to USB CDC (`HAL_USE_SERIAL_USB TRUE`); need to intercept bulk IN writes to EP1/EP2 OUT FIFOs and route them to a host-side console probe (similar to `usart_probe`)
