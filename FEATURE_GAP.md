...
| Runtime now reaches steady USART2 RX DMA re-arm path without warnings | high | Updated | After adding minimal RCC/PWR/FLASH startup semantics, bounded runs no longer stop in `stm32_clock_init()`. A 3M traced run reaches `pc=0x0815ca54` and shows live `OTG_FS` SOF IRQ 67 dispatches plus `TIM5` IRQ 50 pending/dispatch/return (`TIM5 CC1 compare fired` at clk=`02985614`, `NVIC dispatching IRQ 50` at clk=`02985655`). A 120M bounded run completes with no `WARN`, no `ERROR`, and no `peri=????`, ending around `pc=0x08147276`; the hot path symbolizes to `ChibiOS::UARTDriver::dma_rx_enable()` and repeatedly initiates `DMA1` channel 4 reads from `USART2->DR`. This is the current deterministic runtime marker showing active post-startup execution rather than a passive stall. |
...
| Validation snapshot for current state | high | Updated | Current release build passes both bounded validation goals with no `WARN`, no `ERROR`, and no `peri=????` unknown peripheral accesses. Short traced command: `cd cubeblack && /usr/bin/time -f 'real=%e user=%U sys=%S maxrss=%M exit=%x' timeout 30 ../target/release/stm32-emulator config.yaml -v --max-instructions 3000000` → stop at `clk=03000000 pc=0x0815ca54`, `real=1.27s`, with repeated `OTG_FS` IRQ 67 dispatches and a confirmed `TIM5` IRQ 50 wake/dispatch path. Long stability command: `cd cubeblack && /usr/bin/time -f 'real=%e user=%U sys=%S maxrss=%M exit=%x' timeout 120 ../target/release/stm32-emulator config.yaml -v --max-instructions 120000000` → stop at `clk=120000000 pc=0x08147276`, `real=55.87s`. Steady-state activity in the long run is `DMA1` channel 4 repeatedly reading `USART2->DR` from `ChibiOS::UARTDriver::dma_rx_enable()`, which is the current deterministic runtime marker. Remaining gap: no new human-readable USART-probe or USB-CDC console line has been captured in this post-startup state yet. |
