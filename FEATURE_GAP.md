...
| Validation snapshot for current state | high | Partially Implemented | After the memory-map, DMA, NVIC, SCB, timer progression, and minimal DWT fixes, bounded runs remain stable to 5M, 20M, and 100M instructions and no longer stay parked at `0x08161148`. A 5M scan now shows no remaining `peri=????`, warnings, or errors in that window, while a 5M trace shows `TIM5` raising IRQ 50, entering the IRQ handler, and returning to thread mode. Full functional boot is still not demonstrated. CubeBlack hardware definitions also enable USB serial (`HAL_USE_SERIAL_USB`), so missing USART-probe lines alone should not be treated as a hard failure if another deterministic runtime marker is captured. |
