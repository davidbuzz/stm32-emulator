...
| USB OTG FS endpoint/FIFO/data semantics are still incomplete | high | Partially Implemented | A dedicated `src/peripherals/otg_fs.rs` model now exists, clears the earlier `GRSTCTL` polling stall, synthesizes post-init device events so the firmware actively services `OTG_FS` IRQ 67, and now also carries enough EP0 state to get through the first setup plus zero-length IN completion path without re-triggering the same synthetic setup forever. The remaining gap is payload FIFO semantics, richer endpoint state progression, and a CDC-style data bridge that would let USB serial traffic appear outside the emulated MCU. |
| No deterministic runtime marker beyond wake/reschedule plus USB IRQ service is captured yet | high | Partially Implemented | The emulator now runs cleanly to `100000000` instructions, still shows the expected `TIM5` IRQ 50 wake path plus SVCall/reschedule traffic, and now also services repeated `OTG_FS` IRQ 67 events after USB init. It still does not emit a recognizable firmware console line or another stronger runtime milestone, so the next useful work is on runtime-visible USB CDC endpoint/data flow and any remaining USART DMA/runtime state gaps. |
...
| Validation snapshot for current state | high | Partially Implemented | The current release build completes bounded runs to `20M` and `100M` instructions with no `WARN`, no `ERROR`, and no `peri=????` unknown peripheral accesses. After the latest OTG FS endpoint patch, a focused `5M` USB trace shows `DIEPEMPMSK` latching, `DIEPINT0` transitioning from synthetic `TXFE` to `XFRC = 0x1`, and the old repeated EP0 helper/stall frontier no longer dominating the trace. The long run still shows the expected `TIM5` IRQ 50 wake path plus repeated `OTG_FS` IRQ 67 service, but there is still no stronger runtime marker such as USART/USB console output. |
