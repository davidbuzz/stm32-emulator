...
| Minimal OTG FS global/device bring-up added | high | Partially Implemented | `src/peripherals/otg_fs.rs` now handles `OTG_FS_GLOBAL`, `OTG_FS_DEVICE`, and `OTG_FS_PWRCLK` with persisted core registers and minimal `GRSTCTL` idle/reset semantics. This is enough to get past the earlier USB core-reset polling loop and into later USB/DMA-backed runtime paths, but endpoint/FIFO/interrupt behavior is still largely unmodeled. |
...
| USB OTG FS endpoint/FIFO/interrupt semantics are still incomplete | high | Partially Implemented | A dedicated `src/peripherals/otg_fs.rs` model now exists and clears the earlier `GRSTCTL` polling stall by returning `AHBIDL` and auto-clearing reset bits. Execution now advances through USB device configuration and into later DMA-backed paths, but endpoint control/status, FIFO state, and USB interrupt semantics are still too shallow for full bring-up. |
| Post-OTG run now reaches SVC/context-switch restore and faults on unmapped fetch | high | Partially Implemented | With the minimal OTG model in place, a rebuilt release run advances to about `10.64M` instructions and reaches active DMA transfers (`DMA2` from `ADC1`, `DMA1` RX/TX on `SPI2`) before failing with `FETCH_UNMAPPED addr=0x00010002` at `pc=0x0800543c` after `svc 0` handling. Targeted trace instrumentation confirms this occurs at `pop.w {r4-r11,pc}` with a corrupted stacked return word (`0x00010003`), so the next blocker is still scheduler/context frame integrity rather than USB reset polling. |
...
| Validation snapshot for current state | high | Partially Implemented | Before the OTG work, bounded runs stayed stable to 100M instructions but remained stuck in USB-heavy polling with no stronger runtime marker. After adding the minimal OTG FS model and SVCall exception routing, the rebuilt release binary advances materially further into runtime behavior, including observed DMA transfers on `ADC1` and `SPI2`, before failing at `FETCH_UNMAPPED addr=0x00010002` from `pc=0x0800543c`. Full functional boot is still not demonstrated, but the dominant blocker has moved beyond the original OTG core-reset loop. |
...
- P0 (must-have for runtime):
	...
	- USB OTG FS minimal global/device status semantics used by ChibiOS/ArduPilot USB-serial bring-up
