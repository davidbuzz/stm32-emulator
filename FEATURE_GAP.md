...
| Validation snapshot for current state | high | Updated | Current release build passes bounded validation to `200000000` instructions with no emulator `WARN`, no emulator `ERROR`, and no `peri=????` unknown peripheral accesses. Short command: `cd cubeblack && /usr/bin/time -f 'real=%e user=%U sys=%S maxrss=%M exit=%x' timeout 30 ../target/release/stm32-emulator config.yaml -v --max-instructions 3000000` → stop at `clk=03000000 pc=0x0815ca54`, `real=1.66s`, with repeated `TIM5` IRQ 50 dispatches and the usual USB activity. Long command: `cd cubeblack && /usr/bin/time -f 'real=%e user=%U sys=%S maxrss=%M exit=%x' timeout 120 ../target/release/stm32-emulator config.yaml -v --max-instructions 200000000 > ardu.cubeblack.log 2>&1` → stop at `clk=200000000 pc=0x0815ca54`, `real=117.39s`, and refresh `cubeblack/ardu.cubeblack.log`. This run clears the previous `145255239` mixed-state IRQ fault and leaves the evidence log free of emulator warnings/errors. |
